 ...
 * 即便创建了多个实例并在 service.init(mode) 传入了不同的模式都没用，这可能是 Vue-Cli 的 BUG。
 *
 * 所以这里开启多个进程来生成文件。
 */
const [, script, mode, file] = process.argv;
if (mode) {
	...
	execFileSync(process.execPath, [script, "development", "temp/webpack.js"]);
	execFileSync(process.execPath, [script, "production", "temp/webpack.prod.js"]);
}
